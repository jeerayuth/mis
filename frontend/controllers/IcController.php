...
class IcController extends CommonController {
    ...
    public function actionReport1($datestart, $dateend, $details) {
        ...
        $sql = "SELECT
                    ...
                    lh.lab_order_number, lh.order_date,lo.lab_items_code ,
                    if(lo.lab_items_code = 2098,lo.lab_order_result,'')  as CBC,
                    if(lo.lab_items_code = 69,lo.lab_order_result,'')  as BUN ,
                    if(lo.lab_items_code = 3003,lo.lab_order_result,'')  as Creatinine,
                    if(lo.lab_items_code = 3001,lo.lab_order_result,'')  as Glucose_FPG,
                    if(lo.lab_items_code = 3014,lo.lab_order_result,'')  as SGOT_AST,
                    if(lo.lab_items_code = 3015,lo.lab_order_result,'')  as SGPT_ALT,
                    if(lo.lab_items_code = 3016,lo.lab_order_result,'')  as ALP ,
                    if(lo.lab_items_code = 3005,lo.lab_order_result,'')  as Cholesterol,
                    if(lo.lab_items_code = 3007,lo.lab_order_result,'')  as HDL_C,
                    if(lo.lab_items_code = 3008,lo.lab_order_result,'')  as LDL_C,
                    if(lo.lab_items_code = 3006,lo.lab_order_result,'')  as Triglyceride,
                    if(lo.lab_items_code = 48,lo.lab_order_result,'')  as HbA1C ,
                    if(lo.lab_items_code = 2176,lo.lab_order_result,'')  as Anti_HBs ,
                    if(lo.lab_items_code = 2174,lo.lab_order_result,'')  as HBsAg ,
