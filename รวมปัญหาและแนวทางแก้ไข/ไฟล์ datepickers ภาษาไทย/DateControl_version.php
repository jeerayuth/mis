...
class DateControl extends \kartik\base\InputWidget
{
    ...
    protected function getDisplayInput()
    {
        ...
        $valueStr = empty($value) ? '' : explode ( ' ' ,$value);
        $this->options['value'] = empty($valueStr) ? '' : $valueStr[0]. ' '. $valueStr[1].' '.($valueStr[2]+543);
    ...
    protected function getDisplayValue($data)
    {
        ...
        if ($date instanceof DateTime) {
            ...
            //$value = $date->format('dd MM ') . ($date->format('yyyy') + 543).toString;
